...
func TestHandleBootstrapErrorCommand(t *testing.T) {
	...
	dns_result := handle_bootstrap_error(dns_out, dns_host)
	if dns_result != true {
		t.Error("Expected dns error true, got ", dns_result)
	}
	auth_result := handle_bootstrap_error(auth_out, auth_host)
	if auth_result != true {
		t.Error("Expected auth error true, got ", auth_result)
	}
	timeout_result := handle_bootstrap_error(timeout_out, timeout_host)
